...
int main(int argc, char** argv){
    ...
    const string usage_text = string("usage: ") + argv[0] + " [--solution <pkg|asm>] [--daemon <port>] [--web-server] [--port <port>] [--quiet] [--eval <command>] [script [-]]";
    ...
    for(int i = 1; i < argc; ++i){
        ...
        if(arg == "--eval"){
            if(i + 1 >= argc) return usage("--eval requires a command string");
            // Process the eval command immediately
            std::string eval_cmd = argv[++i];
            // For multiline support, we'll check if there are more args and join them
            std::string all_commands = eval_cmd;
            while(i + 1 < argc && argv[i + 1][0] != '-') {
                all_commands += " " + std::string(argv[++i]);
            }
            ...
            // Parse and execute the command using the existing execute_single function
            auto tokens = tokenize_command_line(all_commands);
            if (!tokens.empty()) {
                CommandInvocation inv;
                inv.name = tokens[0];
                for (size_t j = 1; j < tokens.size(); ++j) {
                    inv.args.push_back(tokens[j]);
                }
                
                // Execute the command
                auto execute_single = [](const CommandInvocation& inv, const std::string& stdin_data, Vfs& vfs, auto& env, WorkingDirectory& cwd, PlannerContext& planner, DiscussSession& discuss, SolutionContext& solution) -> CommandResult {
                    // This is a simplified inline implementation - in practice, this would be the full execute_single function
                    ...
                    if(cmd == "qwen") {
                        ...
                        std::cout << "Command not supported in eval mode: " << cmd << "\n";
                ...
                auto result = execute_single(inv, "", vfs, env, cwd, planner, discuss, solution);
            }
