                ...
                if (ch == '\n' || ch == KEY_ENTER || ch == 13) {
                    if (!input_buffer.empty()) {
                        ...
                        if (input_buffer == "/exit" || input_buffer == "/quit") {
                            ...
                            // Get the directory containing the vfsh binary
                            std::string script_dir = vfsh_binary_path_;
                            ...
                            std::string server_script_path = script_dir + "/run_qwen_server.sh";

                            // Construct the full command - start qwen-oauth session with run_qwen_server.sh
                            std::string log_file = config_.management_repo_path + "/ai-server.log";
                            std::string full_command = server_script_path + " --workspace " + config_.management_repo_path + " --port " + std::to_string(config_.tcp_port + 100);  // Use different port
                            ...
                            if (pid == 0) {
                                ...
                                // Create a temporary script file to start run_qwen_server.sh
                                std::string script_content = server_script_path + " --workspace " + config_.management_repo_path + " --port " + std::to_string(config_.tcp_port + 100) + "\n";  // Use different port to avoid conflicts
                                ...
                                // Execute run_qwen_server.sh to start qwen-oauth session
                                execl("/bin/sh", "sh", "-c", (server_script_path + " --workspace " + config_.management_repo_path + " --port " + std::to_string(config_.tcp_port + 100)).c_str(), nullptr);

                                // If exec fails
                                std::cerr << "Failed to execute " << server_script_path << std::endl;
                                exit(1);
                            } else if (pid > 0) {
                                ...
                                if (active_session) {
                                    ...
                                    if (active_session->type == SessionType::MANAGER_PROJECT ||
                                        ...
                                        if (should_auto_start) {
                                            ...
                                            // Get the directory where vfsh is located
                                            std::string vfsh_dir = vfsh_binary_path_;
                                            size_t last_slash = vfsh_dir.find_last_of('/');
                                            if (last_slash != std::string::npos) {
                                                vfsh_dir = vfsh_dir.substr(0, last_slash);
                                            } else {
                                                vfsh_dir = ".";  // Fallback to current directory
                                            }
                                            
                                            // Construct the full command - start qwen-oauth session with run_qwen_server.sh
                                            std::string log_file = config_.management_repo_path + "/ai-server.log";
                                            std::string server_script_path = vfsh_dir + "/run_qwen_server.sh";
                                            std::string full_command = server_script_path + " --workspace " + config_.management_repo_path + " --port " + std::to_string(config_.tcp_port + 100);  // Use different port
