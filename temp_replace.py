...
# Define the pattern to match the error-generating code
pattern = r'has_real_command = false;\s*// Use internal U\+\+ builder when no COMMAND is provided\s*if\(builder\) \{\s*std::string builder_label = builder->id;\s*command_body = "printf \\'%s\\' \\\\\\"upp.wksp.build: builder \\\'\\" \+ builder_label \+\s*"\\' has no COMMAND defined; configure the build method to describe how to build package \\'\\" \+\s*pkg.name \+ "\\'\.\\\\\\" >&2; exit 1";'
